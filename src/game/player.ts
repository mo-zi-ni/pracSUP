...
/**
 * 헛가드 경직(ms). 저스트가드에서 이게 없으면 연타로 전부 막을 수 있어
 * 타이밍 연습 자체가 무의미해진다. 이 값이 시스템의 난이도를 결정한다.
 */
export const GUARD_WHIFF_LOCKOUT = 700;
...
export interface Player {
  ...
  /** 헛가드 경직 남은 시간(ms). 0보다 크면 가드를 쓸 수 없다. */
  guardLockout: number;
...
export function createPlayer(spawn: Vec2 = { x: 0, z: 12 }, maxHp = 3): Player {
  ...
  const player: Player = {
    ...
    update(dt, input, arenaRadius) {
      ...
      this.guardLockout = Math.max(0, this.guardLockout - dt);
    ...
    sync() {
      ...
      // 상태를 색으로 구분한다: 무적(노랑) > 경직(회색) > 평시(하늘색)
      const color = this.invulnerable ? 0xfff08a : this.guardLockout > 0 ? 0x6b7392 : 0x63d2ff;
